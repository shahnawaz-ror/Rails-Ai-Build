    ...
    class Replayer
      SCRIPTS = {
        "health-check" => [
          ...
          { event: "iteration", data: { content: "I'll add a health check endpoint. Let me inspect your routes first.", tool_calls: 1 }, delay: 0.8 },
          ...
          { event: "tool_call", data: { name: "write_file", arguments: { path: "app/controllers/health_controller.rb", content: "# HealthController..." } }, delay: 0.6 },
          { event: "tool_call", data: { name: "write_file", arguments: { path: "config/routes.rb", content: "get '/health', to: 'health#show'" } }, delay: 0.5 },
        ...
        "crud-post" => [
          ...
          { event: "tool_call", data: { name: "shell", arguments: { command: "bin/rails generate model Post title:string body:text" } }, delay: 0.8 },
        ...
        "fix-test" => [
          ...
          { event: "tool_call", data: { name: "read_file", arguments: { path: "spec/requests/users_controller_spec.rb", offset: 35, limit: 15 } }, delay: 0.5 },
          { event: "iteration", data: { content: "Assertion expects 201 but controller returns 200. Checking controller...", tool_calls: 1 }, delay: 0.7 },
          { event: "tool_call", data: { name: "read_file", arguments: { path: "app/controllers/users_controller.rb" } }, delay: 0.5 },
          { event: "tool_call", data: { name: "write_file", arguments: { path: "spec/requests/users_controller_spec.rb", content: "expect(response).to have_http_status(:ok)" } }, delay: 0.6 },
          { event: "tool_call", data: { name: "shell", arguments: { command: "bundle exec rspec spec/requests/users_controller_spec.rb:42" } }, delay: 0.7 },
          { event: "complete", data: {
            role: "assistant",
        ...
        "api-auth" => [
          ...
          { event: "iteration", data: { content: "[Planner] API auth needs: JwtService, sessions#create, before_action on Api::BaseController", tool_calls: 0 }, delay: 0.8 },
          { event: "iteration", data: { content: "[Coder] Implementing JWT encode/decode and protected routes...", tool_calls: 3 }, delay: 0.9 },
          { event: "tool_call", data: { name: "write_file", arguments: { path: "app/services/jwt_service.rb" } }, delay: 0.5 },
          { event: "tool_call", data: { name: "write_file", arguments: { path: "app/controllers/api/sessions_controller.rb" } }, delay: 0.5 },
          { event: "tool_call", data: { name: "write_file", arguments: { path: "app/controllers/api/base_controller.rb" } }, delay: 0.5 },
          { event: "iteration", data: { content: "[Reviewer] Auth flow looks good. Suggest adding request spec for 401 on missing token.", tool_calls: 1 }, delay: 0.7 },
          { event: "complete", data: {
            role: "assistant",
      ...
      class << self
        def play(scenario_id, _scenario, &block)
          script = SCRIPTS[scenario_id] || SCRIPTS["health-check"]

          script.each do |step|
            sleep(step[:delay]) if step[:delay].positive?
            block.call(Streaming::Sse.format_sse(event: step[:event], data: step[:data]))
          end
        end
      end
    end
