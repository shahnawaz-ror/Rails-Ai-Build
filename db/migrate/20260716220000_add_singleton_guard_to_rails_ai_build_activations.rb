...
class AddSingletonGuardToRailsAiBuildActivations < ActiveRecord::Migration[7.1]
  def up
    ...
    # Collapse accidental duplicates before unique index.
    if ActiveRecord::Base.connection.select_value("SELECT COUNT(*) FROM rails_ai_build_activations").to_i > 1
      keeper_id = ActiveRecord::Base.connection.select_value(
        "SELECT id FROM rails_ai_build_activations ORDER BY id ASC LIMIT 1"
      )
